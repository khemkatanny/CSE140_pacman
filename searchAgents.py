    ...
    def successorStates(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if (not hitsWall):
                successors.append((
                    (next_node, (
                        (next_node == self.corners[0]) or state[1][0],
                        (next_node == self.corners[1]) or state[1][1],
                        (next_node == self.corners[2]) or state[1][2],
                        (next_node == self.corners[3]) or state[1][3])
                        ), action, 1))
        self._numExpanded += 1
        return successors
...
def cornersHeuristic(state, problem):
    ...
    node = state[0]
    # to store visited corners
    visited_corner = state[1]
    # to store min dist between heuristic value between
    # corners and current position
    min_dist = [0, 0, 0, 0]
    for i in range(len(corners)):
        min_dist[i] = (abs(node[0] - corners[i][0])
        + abs(node[1] - corners[i][1])) * (not visited_corner[i])
    min_dist.sort()
    return min_dist[3]
